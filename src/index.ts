...
function generateAutoGeneratedComment(templateType: string): string {
  const baseComment = `IMPORTANT: This component is automatically generated by update-vite-tag.
...
export default function viteSrcUpdate({
  ...
  function updateTemplateFileForDevelopment(
    ...
    // Assuming config.build.rollupOptions.input is used for other script paths
    let inputs = config.build.rollupOptions.input;
    if (typeof inputs === "object" && inputs !== null) {
      for (const scriptPath of Object.values(inputs)) {
        const basename = path.basename(scriptPath);
        devComponentContent += `<script type="module" src="${devServerAddress}/${basename}"></script>\n`;
      }
    }
